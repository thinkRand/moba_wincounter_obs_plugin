...
static void worker_thread_func(WorkerHandle *w)
{
	while (true) {
		FrameTask task;
		{
			...
			w->cv.wait(lock, [w]() { return w->shutdown || !w->queue.empty(); });
		...
		enum MatchResult result = detector_process_frame(w->detector, task.data.data(), task.width, task.height,
								 task.stride, &match_info);
		...
		if (result == MATCH_VICTORY || result == MATCH_DEFEAT) {
			if (result == w->last_result) {
				...
				obs_queue_task(OBS_TASK_UI, [](void *) { plugin_update_text_sources(); }, NULL, false);

				w->last_result = result;
				w->last_result_time_ms = now_ms;

				obs_log(LOG_INFO, "counted: w:%ld/l:%ld", atomic_load_long(&g_state.wins),
...
bool worker_push_frame(void *handle_ptr, const uint8_t *y_plane, int width, int height, int stride,
		       uint64_t timestamp_ns)
{
	WorkerHandle *w = static_cast<WorkerHandle *>(handle_ptr);
	if (!w)
