...
void *filter_create(obs_data_t *settings, obs_source_t *context)
{
	...
	trigger_set_victory_scene(
		obs_data_get_string(settings, "victory_trigger_scene"));
	trigger_set_defeat_scene(
		obs_data_get_string(settings, "defeat_trigger_scene"));

	plugin_set_language_key(
		obs_data_get_string(settings, "language_key"));
	plugin_set_game_key(
		obs_data_get_string(settings, "game_key"));

	obs_log(LOG_INFO, "filter_create: src=%p lang='%s' game='%s'",
		(void *)context, g_state.language_key, g_state.game_key);
...
static bool language_modified(obs_properties_t *props,
			      obs_property_t *property, obs_data_t *settings)
{
	...
	if (g_state.detector) {
		...
		for (int i = 0; i < n; i++) {
			const char *key = detector_get_game_key(
				g_state.detector, i);
...
static bool support_button_clicked(obs_properties_t *props,
...
static obs_properties_t *filter_get_properties(void *data)
{
	...
	obs_property_t *lang_list = obs_properties_add_list(props, "language_key",
		"Language", OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
	if (g_state.detector) {
		int n = detector_get_language_count(g_state.detector);
		for (int i = 0; i < n; i++) {
			const char *key = detector_get_language_key(
				g_state.detector, i);
	...
	obs_property_t *game_list = obs_properties_add_list(props, "game_key",
		"Game", OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_STRING);
	if (g_state.detector) {
		int n = detector_get_game_count(g_state.detector);
		for (int i = 0; i < n; i++) {
			const char *key = detector_get_game_key(
	...
	obs_properties_add_text(props, "victory_trigger_scene",
		"Trigger Scene on Victory", OBS_TEXT_DEFAULT);
	obs_properties_add_text(props, "defeat_trigger_scene",
		"Trigger Scene on Defeat", OBS_TEXT_DEFAULT);

	obs_properties_add_button(props, "support_button",
		"Support on Ko-fi", support_button_clicked);
