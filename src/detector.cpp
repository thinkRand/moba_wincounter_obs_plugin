...
static bool load_game_templates(DetectorHandle *handle,
                                const std::string &lang_key,
                                const std::string &game_key,
                                const std::string &victory_path,
                                const std::string &defeat_path)
{
    GameTemplates gt;
    ...
    handle->sift->detectAndCompute(gt.victory_img, cv::noArray(),
                                   gt.victory_kp, gt.victory_des);
    handle->sift->detectAndCompute(gt.defeat_img, cv::noArray(),
                                   gt.defeat_kp, gt.defeat_des);
    ...
    std::fprintf(stderr,
                 "[moba_wincounter] Loaded '%s/%s': victory=%dx%d %dkp, defeat=%dx%d %dkp\n",
                 lang_key.c_str(), game_key.c_str(),
                 stored.victory_img.cols, stored.victory_img.rows,
                 (int)stored.victory_kp.size(),
                 stored.defeat_img.cols, stored.defeat_img.rows,
                 (int)stored.defeat_kp.size());
    return true;
}

static bool load_templates_from_dir(DetectorHandle *handle,
                                    const std::string &lang_key,
                                    const std::filesystem::path &dir)
{
    ...
    for (const auto &entry : fs::directory_iterator(dir)) {
        ...
        if (!fs::is_regular_file(defeat_path)) {
            std::fprintf(stderr,
                         "[moba_wincounter] Skipping '%s/%s': missing '%s'\n",
                         lang_key.c_str(), filename.c_str(),
                         defeat_name.c_str());
            continue;
        }

        if (load_game_templates(handle, lang_key, game_key,
                                entry.path().string(),
                                defeat_path.string())) {
            count++;
        }
...
void *detector_create(const char *template_dir)
{
    ...
    if (!first_lang.game_order.empty()) {
        ...
        handle->active.store(&handle->languages[handle->current_language]
                                  .games[handle->current_game]);
    }
                                        ...
                                        struct FrameMatchInfo *info)
{
    ...
    if (!frame_des.empty() && frame_kp.size() >= 2) {
        inliers_v = match_template_sift(handle, img_gray,
                                        active->victory_kp,
                                        active->victory_des,
                                        active->victory_img.cols,
                                        active->victory_img.rows,
                                        frame_kp, frame_des,
                                        &good_v);
        inliers_d = match_template_sift(handle, img_gray,
                                        active->defeat_kp,
                                        active->defeat_des,
                                        active->defeat_img.cols,
                                        active->defeat_img.rows,
                                        frame_kp, frame_des,
                                        &good_d);
    }
